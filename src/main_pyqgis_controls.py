    ...
    for feature in iterador_features:
        ...
        while (vertices_feature.hasNext() and (not tiene_error)):
            ...
            if ((cantidad_vertices == 2) or pedir_nuevo_punto):
                ...
                if (flujo == _('down')):                        
                    if (punto_vertice.z() - altura_anterior > args.t1) or (punto_vertice.z() - min_altura > args.t1):
                        ...
                        fman.append_csv_file(nom_sal, [capa1, feature['id'], texto_imprimir, punto_vertice.z(), abs(dif_imprimir),  punto_vertice.x(), punto_vertice.y()])
                    if (punto_vertice.z() < min_altura):
                        min_altura = punto_vertice.z()
                else: 			
                    if (punto_vertice.z() - altura_anterior < -1 * args.t1) or (punto_vertice.z() - max_altura < -1 * args.t1):
...
def control(capa_verificar, c, l_capas_interectar, u, lindx, n_sal, args, df, l_continuity, fman): 
    ''' Return two list. The first list has the features to be possible max height , and the second list possible endorreics. '''               
    posible_endorreica = []
    posible_cota_1 = []
    posible_cota_2 = []
    contador = 0        
    ...
    hrow = [_('Input_Layer'), _('OBJECTID'), _('Description'), _('Height'), _('Height_difference'), _('X_Coordinate'), _('Y_Coordinate')]
    ...
    for feature in it_features:
        ...
        if (not geometria.isNull() and not geometria.isEmpty()):
            ...
            (c_1, c_2) = interseccion_misma_z(c, u, primer_vertice, ultimo_vertice, fman, n_sal, feature_id, indice_capa, args, df, lindx, lc_inter) 
...
def interseccion_misma_z(in_capa, in_uri, v1, v2, fman, n_out, fid, indx, args, df, lindx, lc_inter):    
...
def interseccion_misma_z2(c, g, altura, fid, fman, n_out, args):
    ...
    if (g.wkbType() == QgsWkbTypes.PointZ):
        if (abs(g.constGet().z() - altura) >= args.t2):
            fman.append_csv_file(n_out, [c, fid, _('Error - Difference in height intersection'), altura, abs(g.constGet().z() - altura), g.constGet().x(), g.constGet().y()])
    ...
    # Polyline intersection
        ...
        while (it_vertices.hasNext() and (not has_error)):
            ...
            if (abs(p_vertice.z() - altura) >= args.t2):
                fman.append_csv_file(n_out, [c, fid, _('Error - Difference in height intersection'), altura, abs(p_vertice.z() - altura), p_vertice.x(), p_vertice.y()])
                has_error = True
    ...
    # MultiPoint intersection
        ...
        for i in range(num_geom):
            ...
            if (abs(punto.z() - altura) >= args.t2):
                fman.append_csv_file(n_out, [c, fid, _('Error - Difference in height intersection'), altura, abs(punto.z() - altura), punto.x(), punto.y()])
                break    
...
def is_max_height(c, c_nom, l_fid, l_ci, l_index, args):   
    ...
    for feat_id in it_feat3:        
        ...
        if (encontre):
            ...
            if not ritc:
                resultado.append([c_nom, feat_id['id'], _('Error - Node with no maximum height'), '', '', '', ''])
        count = count + 1 
    return resultado 
...
def control_4(capa_4, uri, indices, args, nam_sal, lista_intersectar, fman):   
    ...
    hrow = [_('Input_Layer'), _('OBJECTID'), _('Description'), _('Intersection_Layer'), _('OBJECTID'), _('Height'), _('Height_difference'), _('X_Coordinate'), _('Y_Coordinate')]
    ...
    for feature in iterador_features:
        ...
        while (vertices_feature.hasNext()):
            ...
            if (abs(alt_total - alt_actual) >= args.t2):
                fman.append_csv_file(nam_sal,[capa_4, feature['id'], _('Error - Polygon height'), '', '', alt_total, abs(alt_total - alt_actual), punto_vertice.x(), punto_vertice.y()])							                    
                hay_error = True
        # Verify the intersection has the same height
        if (not hay_error):                
            for capa in lista_intersectar:			
                intersectar_capa(capa, geometria_feature, alt_total, capa_4, feature['id'], uri, indices, args, fman, nam_sal)        
												
def intersectar_capa(c, g_f, altura_pol, c_original, fea_original, uri, indexs, args, fman, nam_sal):		
        ...
        if (g_f.intersects(f.geometry())):  
            ...
            if (geom_interseccion.wkbType() == QgsWkbTypes.PointZ):
                if (abs(geom_interseccion.get().z() - altura_pol) >= args.t2):
                    fman.append_csv_file(nam_sal, [c_original, fea_original, _('Error - Intersection  height'), c, f['id'], altura_pol, altura_pol - geom_interseccion.get().z()])						
            ...
            # Interseccion linea o multilinea
                ...
                while (it_vertices.hasNext() and (not hay_error)):
                    ...
                    if (abs(p_vertice.z() - altura_pol) >= args.t2):
                        fman.append_csv_file(nam_sal, [c_original, fea_original, _('Error - Intersection  height'), c, f['id'], altura_pol, altura_pol - p_vertice.z()])						
                        hay_error= True
            ...
            # Interseccion multipunto			
                ...
                for i in range(num_geom):
                    ...
                    if (abs(punto.z() - altura_pol) >= args.t2):
                        fman.append_csv_file(nam_sal, [c_original, fea_original, _('Error - Intersection  height'), c, f['id'], altura_pol, altura_pol - punto.z()])						
                        break
            else:
...
if __name__ == '__main__':        
    ...
    fman = initFileManager(args.output)        
    ...
    for name_l_flow in f_config["flujo"]:
        ...
        result_name = args.dbschema + '_'  + date_time + 'Control_Vertex_Height_' + name_l_flow +'.csv'
        cotas, endorreicas = control(layer_check, name_l_flow, f_config["endorreicas"], uri, l_ind, result_name, args, d_feat, l_continuity, fman)     
        r_cota = is_max_height(layer_check, name_l_flow, cotas, f_config["endorreicas"], l_ind, args)
        r_endo = is_endorreics(name_l_flow, endorreicas, uri, consignment_geometry, l_ind, f_config["endorreicas"], args)        
    ...
    for name_l_constant_height in f_config["altura_area"]:        
        ...
        result_name = args.dbschema + '_' + date_time + 'Control_Polygon_Height_' + name_l_constant_height +'.csv'
